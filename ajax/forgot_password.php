...
// Handles all 3 steps of forgot password via AJAX/POST
// step: 'email' | 'answer' | 'reset'
...
if ($step === 'email') {
    ...
    try {
        ...
        // FIX: Track answer attempts to prevent brute force
...
if ($step === 'answer') {
    ...
    // FIX: Rate limit — max 5 attempts before they must start over
    $attempts = $_SESSION['fp_answer_attempts'] ?? 0;
    if ($attempts >= 5) {
        // Clear the session so they must re-enter email
